# JVM内存区域划分？OOM发生区域？
>
- 通常可以把 JVM 内存区域分为下面几个方面，其中，有的区域是**以线程为单位**，而有的区域则是**整个 JVM 进程唯一**的。
>
...
>
### Java 对象是不是都创建在堆上的呢？
- 注意到有一些观点，认为通过**逃逸分析**，JVM 会在栈上分配那些不会逃逸的对象，这在理论上是可行的，但是取决于 JVM 设计者的选择。
- Oracle Hotspot JVM 中并未这么做，这一点在逃逸分析相关的文档里已经说明，所以可以明确所有的对象实例都是创建在堆上。
>
...
>
##  Java 里面存在哪些种 OOM 的可能性，分别对应哪个内存区域的异常状况呢？
...
- 在专栏第 4 讲的引用机制分析中，已经提到了 JVM 会去尝试回收软引用指向的对象等。
...
>
- 堆上空间大于100M，表示的是新生代和老年代加起来总和大于100M，而新生代和老年代各自并没有大于100M的连续空间。
- 进一步，大数组一般直接进入老年代，是否可以认为老年代中没有连续大于100M的空间？
